...
const SAMPLES: NotifyEvent[] = [
  ...
  { type: 'ci', project: 'zabukai-app', status: 'fail', branch: 'master', commitTitle: XSS, actor: 'x' },
  { type: 'pr', project: 'zabukai-app', action: 'opened', number: 1, title: XSS, author: XSS },
  { type: 'issue', project: 'zabukai-app', action: 'opened', number: 1, title: XSS, assignee: XSS },
  { type: 'incident', project: 'zabukai-app', title: XSS, detail: XSS },
...
test('the tags are the first line, BEFORE the text, not after', () => {
  const text = render({ type: 'ci', project: 'zabukai-app', status: 'ok', branch: 'master' });
...
const VOCABULARY: Array<[NotifyEvent, string]> = [
  [{ type: 'deploy', project: 'zabukai-app', status: 'ok' }, ICON.ok],
  [{ type: 'deploy', project: 'zabukai-app', status: 'fail' }, ICON.red],
  [{ type: 'ci', project: 'zabukai-app', status: 'ok' }, ICON.ok],
  [{ type: 'ci', project: 'zabukai-app', status: 'fail' }, ICON.red],
  [{ type: 'job', project: 'zabukai-app', job: 'x', status: 'ok' }, ICON.ok],
  [{ type: 'job', project: 'zabukai-app', job: 'x', status: 'fail' }, ICON.red],
  [{ type: 'job', project: 'zabukai-app', job: 'x', status: 'disabled' }, ICON.off],
  [{ type: 'job', project: 'zabukai-app', job: 'x', status: 'silent' }, ICON.unknown],
  [{ type: 'pr', project: 'zabukai-app', action: 'opened', number: 1, title: 't' }, ICON.fresh],
  [{ type: 'pr', project: 'zabukai-app', action: 'approved', number: 1, title: 't' }, ICON.approved],
  [{ type: 'pr', project: 'zabukai-app', action: 'changes_requested', number: 1, title: 't' }, ICON.changes],
  [{ type: 'pr', project: 'zabukai-app', action: 'merged', number: 1, title: 't' }, ICON.landed],
  [{ type: 'pr', project: 'zabukai-app', action: 'closed', number: 1, title: 't' }, ICON.discarded],
  ...
  [{ type: 'heartbeat_miss', project: 'zabukai-app', job: 'x', recovered: true }, ICON.ok]
];
...
test('type line: the icon stays outside the bold, no name is invented, the outcome is spelled out', () => {
  const text = render({ type: 'ci', project: 'zabukai-app', status: 'ok' });
...
test('field: a bold label with a capital letter, the value as plain text', () => {
  const text = render({ type: 'job', project: 'zabukai-app', job: 'x', status: 'fail', note: 'причина' });
...
test('incident: the log is a monospaced local path, not a link — and one label', () => {
  const text = render({ type: 'incident', project: 'zabukai-app', title: 'x', detail: 'reason', logs: '~/.claude/logs/' });
...
test('the clamp does not leave an unclosed tag on any form', () => {
  ...
  const cases: Array<[string, NotifyEvent]> = [
    ...
    ['issue body', { type: 'issue', project: 'zabukai-app', action: 'opened', number: 1, title: 'x', body: long }],
...
test('issue/pr: the number is the link on line 2, first, and never disappears', () => {
  ...
  const linked = render({
    type: 'issue', project: 'zabukai-app', action: 'assigned', number: 128,
...
test('a multiline commit title is cut to its first line (protects against the full body in a field)', () => {
  ...
  const ci = render({ type: 'ci', project: 'zabukai-app', status: 'ok', commit: '9b1fc68', commitTitle: messy });
...
test('workflow with no url — there is no row at all, not "Check: run" without a link', () => {
  const out = render({ type: 'ci', project: 'zabukai-app', status: 'ok' });
...
test('run: what ran is the type line itself and carries the link', () => {
  ...
  const ci = render({
    type: 'ci', project: 'zabukai-app', status: 'fail', branch: 'master',
...
test('severity: job disabled rings like fail, heartbeat recovered stays quiet like success', () => {
  assert.equal(severity({ type: 'job', project: 'zabukai-app', job: 'x', status: 'disabled' }), 'error');
  assert.equal(severity({ type: 'heartbeat_miss', project: 'zabukai-app', job: 'x', recovered: true }), 'info');
  assert.equal(severity({ type: 'heartbeat_miss', project: 'zabukai-app', job: 'x' }), 'error');
});
...
test('people rows: the owner is printed like anyone else — Author, Assignee, Reviewer, Actor', () => {
  const pr = render({
    type: 'pr', project: 'zabukai-app', action: 'opened', number: 1, title: 'x',
  ...
  const issue = render({
    type: 'issue', project: 'zabukai-app', action: 'assigned', number: 322, title: 'x',
  ...
  const ci = render({ type: 'ci', project: 'zabukai-app', status: 'ok', actor: '@chelsnebes', commit: 'a', commitUrl: 'https://x/c' });
  ...
  const deploy = render({
    type: 'deploy', project: 'zabukai-app', status: 'ok', commit: 'a', commitUrl: 'https://x/c',
...
test('card/issue: assigned carries the body too, quoted', () => {
  const out = render({
    type: 'issue', project: 'zabukai-app', action: 'assigned', number: 312,
...
test('quote: a body longer than a screenful collapses, by length OR by lines', () => {
  const byLength = render({
    type: 'issue', project: 'zabukai-app', action: 'opened', number: 1, title: 't',
    body: 'x'.repeat(500)
  });
...
test('fieldPerson: a multi-line login does not put a raw newline inside the href', () => {
  ...
  const person = render({
    type: 'issue', project: 'zabukai-app', action: 'assigned', number: 1, title: 'x',
    assignee: 'alice\nbob'
  });
...
test('markdown: a GitHub body is translated into Telegram markup, never pasted raw', () => {
  ...
  const card = render({ type: 'issue', project: 'zabukai-app', action: 'opened', number: 1, title: 't', body: body.repeat(40), url: 'https://x/i/1' });
...
test('card with a file: the caption is clamped at 1024, not 4000', () => {
  const out = render({
    type: 'job', project: 'zabukai-app', job: 'Eval: bot answer quality', status: 'ok',
...
test('fieldLink: a multi-line value does not become multi-line link text', () => {
  ...
  // <a> tag.
  const out = render({
    type: 'ci', project: 'zabukai-app', status: 'ok',
...
test('cut marker: with no log, a cut card points at the link on line 2', () => {
  ...
  const out = render({ type: 'pr', project: 'zabukai-app', action: 'merged', number: 315,
  ...
  const nowhere = render({ type: 'pr', project: 'zabukai-app', action: 'merged', number: 315,
...
test('cut marker: a caption card announces the attachment as the full text and fits 1024 (v2.1)', () => {
  ...
  const out = render({ type: 'report', project: 'zabukai-app', title: 'Full dialogues',
...
test('a closed card keeps the subsections of its first section', () => {
  ...
  const out = render({
    type: 'issue', project: 'zabukai-app', action: 'closed', number: 7, title: 't',
...
test('the clamp does not leave a fenced block open', () => {
  ...
  const card = render({ type: 'issue', project: 'zabukai-app', action: 'opened', number: 1, title: 't', body, url: 'https://x/i/1' });
