...
export const CARDS = [
  {
    ...
    note: "Чем выкатили — сама вторая строка, и имя прогона в ней — ссылка на прогон. Строка Commit: хеш — ссылка на коммит, рядом через точку то, что коммит сделал. Строки Source внизу больше нет: ссылка лежит на том, что она открывает."
  },
  {
  ...
  },
  {
    ...
    note: "Серая цитата — тело коммита, она появляется, только если автор его написал. Коммит без тела на этой машине больше не создаётся: гард стоит в 25 репозиториях из 30. Прогона у ручной выкатки нет, поэтому единственная ссылка на карточке — хеш коммита в строке Commit."
  },
  {
  ...
  },
  {
    ...
    note: "Тег #off, а не #fail: сторож выключил прогоны нарочно, это не поломка. Слово «выключено» говорит значок 🚫, поэтому строки State на карточке нет. Страница расхода минут открывается по имени сторожа во второй строке."
  },
  {
  ...
  },
  {
    ...
    note: "Вторая строка — «Issue #322 · заголовок»: номер первым, и он ссылка. Значок говорит, что случилось: 🆕 завели, 🙋 назначили, ✅ закрыли. Тело — под заголовком, переведено из разметки GitHub; при закрытии — только первый раздел. Автор печатается, только когда это не владелец."
  },
  {
  ...
  },
  {
    ...
    note: "Тег #unknown, а не #fail — молчание не значит поломку. Куда смотреть есть и здесь: у задачи с сервера ссылка лежит на её имени во второй строке, у задачи с мака ссылки нет, поэтому там стоит Check с командой."
  },
  {
