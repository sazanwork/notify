...
export const CARDS = [
  ...
  {
    id: 'deploy-nobody', title: 'Выкатка, у коммита нет тела',
    forum: 'PlayHub · Game Publisher',
    when: 'то же событие — но коммит написан без тела',
    live: ['new', 'это и был твой скриншот'],
    sender: 'scripts/deploy.sh',
    expectTag: '#deploy #playhub',
    event: {
      type: 'deploy', project: 'playhub', status: 'fail',
      commit: '9e2e251', commitUrl: 'https://github.com/sazanwork/playhub/commit/9e2e251',
      commitTitle: 'chore(deps): bump @mikitasazan/notify to 1.4.1',
      via: 'manual, from the Mac'
    },
    note: 'Цитаты нет, потому что у самого коммита нет тела. Карточка показала всё, что было. Гард в каждом репозитории машины больше не даёт закоммитить без тела — значит такой карточки не будет.'
  },
  {
    ...
    event: {
      ...
      items: [{ text: 'игры для мальчиков — 4 clicks, pos. 12' }],
      url: 'https://github.com/sazanwork/game-publisher/blob/master/docs/analytics/2026-08-22.md'
    },
    note: 'Ссылка переехала с отдельной строки «Details: open» на само название отчёта. И тег: раньше отчёт не передавал ключ, поэтому тег собирался из заголовка с датой и был новым КАЖДЫЙ день — пара «сломалось → починилось» по нему не находилась никогда. Теперь ключ постоянный.'
  },
  {
