...
- Без `link_preview_options: {"is_disabled": true}` (или legacy
  ...
  этот параметр на обычных сообщениях — проверил, он там есть, добавлять
  его туда не нужно. Реальная дыра — `sendFileOnce` (`send.ts:218–234`,
  подписи к файлам): там этого параметра нет вообще, это и есть настоящий
  пункт для переноса, не выдуманный.
...
- `link_preview_options: {"is_disabled": true}` добавить ТОЛЬКО в
  `sendFileOnce` (`send.ts:218–234`) — в обычных сообщениях он уже есть
  (`send.ts:38`), добавлять туда второй раз не нужно.
