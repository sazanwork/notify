...
(`mac`, `vps`, `actions`), это встаёт в скобку у слова Job: `Job (Mac): config
sync`; и `--took` — сколько прогон занял, твоими же словами (`4m 12s`), строкой
`Took:` под причиной. Не передал — ни скобки, ни строки.
